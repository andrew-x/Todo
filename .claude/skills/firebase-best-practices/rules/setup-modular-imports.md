...
Always import Firebase functions from their specific sub-packages (`firebase/auth`, `firebase/firestore`, etc.). The modular Firebase SDK is designed for tree-shaking, but only when you import from the correct entry points. Importing from the root `firebase` package or using the compat layer pulls in the entire SDK, drastically increasing bundle size.
